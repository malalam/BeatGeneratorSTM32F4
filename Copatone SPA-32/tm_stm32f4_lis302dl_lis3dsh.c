...
void TM_LIS302DL_LIS3DSH_INT_ReadSPI(uint8_t* data, uint8_t addr, uint8_t count) {
	...
	TM_SPI_ReadMulti( LIS302DL_LIS3DSH_SPI, data, placeholder, count );
...
void TM_LIS3DSH_INT_ReadAxes(TM_LIS302DL_LIS3DSH_t *Axes_Data) {
	/* Local variables */
	int8_t axes_temp[ 6 ]; //2 byes for each axis

	/* Read data from sensor over SPI using 
	the TM_LIS302DL_LIS3DSH_INT_ReadSPI function */
	TM_LIS302DL_LIS3DSH_INT_ReadSPI( (uint8_t*)&axes_temp[0], LIS3DSH_OUT_X_L_ADDR, 1 );
	TM_LIS302DL_LIS3DSH_INT_ReadSPI( (uint8_t*)&axes_temp[1], LIS3DSH_OUT_X_H_ADDR, 1 ); 	
	TM_LIS302DL_LIS3DSH_INT_ReadSPI( (uint8_t*)&axes_temp[2], LIS3DSH_OUT_Y_L_ADDR, 1 ); 
	TM_LIS302DL_LIS3DSH_INT_ReadSPI( (uint8_t*)&axes_temp[3], LIS3DSH_OUT_Y_H_ADDR, 1 ); 
	TM_LIS302DL_LIS3DSH_INT_ReadSPI( (uint8_t*)&axes_temp[4], LIS3DSH_OUT_Z_L_ADDR, 1 );
	TM_LIS302DL_LIS3DSH_INT_ReadSPI( (uint8_t*)&axes_temp[5], LIS3DSH_OUT_Z_H_ADDR, 1 );
	

	/* Put received data into Axes_Data */
	(*Axes_Data).X = (int16_t)(axes_temp[0] + (axes_temp[1] << 8))*TM_LIS3DSH_INT_Sensitivity;
	(*Axes_Data).Y = (int16_t)(axes_temp[2] + (axes_temp[3] << 8))*TM_LIS3DSH_INT_Sensitivity; 
	(*Axes_Data).Z = (int16_t)(axes_temp[4] + (axes_temp[5] << 8))*TM_LIS3DSH_INT_Sensitivity;
}
